...
def write_summary(records: Sequence[Record], output_dir: Path) -> None:
    lines = ["序号|Marks|Scored Time|CPU Frequency"]
        ...
        lines.append(
            "|".join(
                (
                    ...
                    f"{format_number(record.cpu_frequency_mhz)} MHz",
                )
            )
        )
...
def statistics_rows(
    ...
    *,
    include_cpu_frequency: bool = False,
    ...
    if include_cpu_frequency:
        rows.append(
            (
                "CPU Frequency (MHz)",
                calculate_statistics(
                    [record.cpu_frequency_mhz for record in records]
                ),
            )
        )
    return tuple(rows)
...
def append_statistics_section(
    ...
    *,
    ...
    for name, result in statistics_rows(
        records, include_cpu_frequency=include_cpu_frequency
    ):
        lines.append(
            "|".join(
                (
...
def write_statistics(
    ...
    *,
    ...
    append_statistics_section(
        ...
        "全部样本",
        records,
        include_cpu_frequency=include_cpu_frequency,
    )
    lines.append("")
    append_statistics_section(
        lines,
...
def write_plots(
    ...
    plot_series(
        ...
        "CPU Frequency",
        nemu_variant,
        output_dir / "cpu_frequency_raw.png",
    )
    ...
    plot_series(
        trimmed_sequence,
        [record.cpu_frequency_mhz for record in trimmed_records],
        "CPU Frequency (MHz)",
        "CPU Frequency ",
        nemu_variant,
        output_dir / "cpu_frequency.png",
    )


def build_parser(
    description: str = "提取单 CPU 测试日志并生成汇总、统计和折线图。",
