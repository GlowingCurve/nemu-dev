...
word_t paddr_read(paddr_t addr, int len) {
  ...
  if ((addr >= CONFIG_MTRACE_START) && (addr <= CONFIG_MTRACE_END)) {
    printf("[MTRACE]: PC:0x%08x READ Addr:0x%08x Len:%d\n", cpu.pc, addr, len);
    log_write("[MTRACE]: PC:0x%08x READ Addr:0x%08x Len:%d\n", cpu.pc, addr,
              len);
  }
#endif
...
#ifdef CONFIG_MTRACE
  if ((addr >= CONFIG_MTRACE_START) && (addr <= CONFIG_MTRACE_END)) {
    printf("[MTRACE]: PC:0x%08x WRITE Addr:0x%08x Len:%d Data:0x%08x\n", cpu.pc,
           addr, len, data);
    log_write("[MTRACE]: PC:0x%08x WRITE Addr:0x%08x Len:%d Data:0x%08x\n",
